        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="/partenaires" class="nav-item nav-link active"><i class="fa fa-th me-2"></i>Partenaires</a>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-sm-6 col-xl-3">
                        <div class="bg-secondary rounded d-flex align-items-center justify-content-between p-4">
                            ...
                            <div class="ms-3">
                                <p class="mb-2">Partenaires</p>
                                <h6 class="mb-0">{{$nombrePartenaires }}</h6>
                            </div>
                        </div>
                    </div>
    ...
    <div class="bg-secondary rounded h-100 p-4">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h6 class="mb-0">Partenaires</h6>
             ...
             <form class="d-none d-md-flex ms-4">
                    <input id="searchInputPartenaires" class="form-control bg-dark border-0" type="search" placeholder="Search">
                </form>

            <a style="font-size: 24px;" title="Ajouter" href="/ajoutpartenaire"><i class="bi bi-person-plus-fill" ></i></a>
        </div>
        <div class="table-responsive">
            <table class="table" id="TablePartenaires">
                <thead>
                    <tr>
                        <th scope="col"></th>
                        <th scope="col">Nom</th>
                ...
                <tbody>
                    @foreach ($partenaires as $partenaire)
                        <tr>
                            ...
                            <td>
                                 <!-- Condition pour afficher l'image par défaut ou l'image du client -->
                @if (!empty($partenaire->image))
                    <img src="{{ asset('expert/img/' . $partenaire->image) }}" alt="Image du client" class="img-fluid rounded-circle mb-3" style="width: 40px; height: 40px;">
                @else
                ...
                @endif
                            ...
                            <td>{{ $partenaire->categorie_id }}</td>
                            <td>{{ $partenaire->ville }}</td>
                            <td>{{ $partenaire->statut }}</td>
                            <td>
                                <!-- Actions pour les partenaires -->
                                <!-- Modifier -->
                                <a class="btn btn-sm btn-success" href="#"><i class="bi bi-pen"></i></a>
                                <!-- Voir -->
                                <a class="btn btn-sm btn-success" title="voir" data-bs-toggle="modal" data-bs-target="#partenaireModal{{ $partenaire->id }}" ><i class="bi bi-eye"></i></a>
                                <!-- Désactiver -->
                                @if($partenaire->statut==='actif')

                                <a title="désactiver" class="btn btn-sm btn-primary" href="/changerStatutCP/{{$partenaire->id}}"><i class="bi bi-person-x"></i></a>
                                @else
                                <a title="activer" class="btn btn-sm btn-success" href="/changerStatutCP/{{$partenaire->id}}"><i class="bi bi-person-check-fill"></i></a>
                                @endif
                            </td>
                        </tr>
...
<div class="modal fade" id="partenaireModal{{ $partenaire->id }}" tabindex="-1" aria-labelledby="partenaireModalLabel{{ $partenaire->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="partenaireModalLabel{{ $partenaire->id }}">Informations du partenaire</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body text-center">
                <!-- Ajouter l'image du client -->
                 @if (!empty($partenaire->image))
                    <img src="{{ asset('expert/img/' . $partenaire->image) }}" alt="Image du partenaire" class="img-fluid rounded-circle mb-3" style="width: 150px; height: 150px;">
                @else
                ...
                <p><strong>Prénom :</strong> {{ $partenaire->prenom }}</p>
